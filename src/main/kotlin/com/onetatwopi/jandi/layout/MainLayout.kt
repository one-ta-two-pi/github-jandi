...
class MainLayout : ToolWindowFactory, DumbAware {
    ...
    private fun generateIssue(jsonIssue: JsonObject) = IssueInfo(
        ...
        openAt = parseDateTime(jsonIssue["created_at"]?.jsonPrimitive?.contentOrNull ?: ""),
        closeAt = parseDateTime(jsonIssue["closed_at"]?.jsonPrimitive?.contentOrNull ?: ""),
    )
    ...
    private fun parseDateTime(dateString: String?): LocalDateTime {
        return dateString?.let {
            try {
                ...
                LocalDateTime.ofInstant(Instant.from(formatter.parse(it)), ZoneOffset.UTC)
            } catch (e: DateTimeParseException) {
                throw RuntimeException("날짜 변환에 실패하였습니다.")
            }
        } ?: LocalDateTime.MIN
    }
}